...
export default function SynagogueConnectionGraph({ synagogues, currentUser, userRole }: SynagogueConnectionGraphProps) {
  const [selectedCity, setSelectedCity] = React.useState<string>(currentUser?.city || '');
    ...
    <div className="bg-white p-6 rounded-lg shadow mt-8">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-4">
          {selectedCity && (
            <button
              onClick={() => navigate(`/admin/connection-status?city=${selectedCity}`)}
      ...
      <div className="flex items-center">
        ...
        <div className="w-1/2 pr-8">
          <div className="space-y-4">
            <div>
              <h4 className="text-sm font-medium text-gray-500 mb-1">בתי כנסת מחוברים</h4>
              <p className="text-2xl font-bold text-green-500">{data[0].value} מתוך {data[0].value + data[1].value}</p>
              <p className="text-sm text-gray-500">מחוברים למערכת</p>
            </div>
            <div>
              <h4 className="text-sm font-medium text-gray-500 mb-1">בתי כנסת לא מחוברים</h4>
              <p className="text-2xl font-bold text-red-500">{data[1].value} מתוך {data[0].value + data[1].value}</p>
              <p className="text-sm text-gray-500">מנותקים מהמערכת</p>
            </div>
            <div className="pt-4">
              <p className="text-sm text-gray-600">
                * הגרף מציג את מצב החיבור הנוכחי של בתי הכנסת{selectedCity ? ` בעיר ${selectedCity}` : ''}
              </p>
              {emergencyMessages.length > 0 && (
                <div className="mt-4 p-4 bg-red-50 rounded-lg">
                  <h4 className="text-sm font-medium text-red-800 mb-2">
                    הודעות חירום פעילות {selectedCity ? `בעיר ${selectedCity}` : ''}
                    {messageStats && (
                      <>
                        <p className="text-sm text-red-600 mt-2">
                          בתי כנסת שקראו את כל ההודעות: {messageStats.readAll} מתוך {messageStats.total}
                        </p>
                        <p className="text-sm text-red-600">
                          בתי כנסת שטרם קראו את כל ההודעות: {messageStats.unread}
                        </p>
                      </>
                    )}
                  </h4>
