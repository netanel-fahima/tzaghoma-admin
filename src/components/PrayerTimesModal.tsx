...
import type { Synagogue, PrayerTime, PrayerType, DayType, TimeRelation, TimeType } from '../types';
...
export default function PrayerTimesModal({ synagogue, onClose }: PrayerTimesModalProps) {
  const [prayerTimes, setPrayerTimes] = useState<PrayerTime[]>(
    (synagogue.prayerTimes || []).map((time, index) => ({ ...time, order: index }))
  );
  ...
  const handleUpdatePrayerTime = (id: string, updates: Partial<PrayerTime>) => {
    setPrayerTimes(prayerTimes.map(time => 
  ...
  return (
    <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-4 sm:p-8 max-w-7xl w-full mx-4 shadow-2xl overflow-hidden" dir="rtl">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center">
            <h3 className="text-xl sm:text-2xl font-bold text-gray-900">ניהול זמני תפילות</h3>
            <span className="mr-2 sm:mr-3 text-sm text-gray-500 hidden sm:inline">({synagogue.name})</span>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6 max-h-[calc(100vh-200px)] overflow-y-auto">
                        ...
                        >
                          <div className="grid grid-cols-1 sm:grid-cols-7 gap-4 items-center flex-grow">
                            <div>
                              <select
                                value={time.dayType}
                                onChange={(e) => handleUpdatePrayerTime(time.id, { dayType: e.target.value as DayType })}
                            ...
                            <div>
                              <select
                                ...
                                onChange={(e) => handleUpdatePrayerTime(time.id, { prayerType: e.target.value as PrayerType })}
                                className="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white py-2 px-3 text-gray-900 appearance-none cursor-pointer bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' fill=\'none\' viewBox=\'0 0 20 20\'%3E%3Cpath stroke=\'%236B7280\' stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'1.5\' d=\'m6 8 4 4 4-4\'/%3E%3C/svg%3E')] bg-[length:1.25em_1.25em] bg-[right_0.5rem_center] bg-no-repeat pr-10"
                            ...
                            <div>
                              <input
                                ...
                                onChange={(e) => handleUpdatePrayerTime(time.id, { description: e.target.value })}
                            ...
                            <div>
                              <select
                                value={time.timeType}
                                onChange={(e) => handleUpdatePrayerTime(time.id, { timeType: e.target.value as TimeType })}
                                className="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white py-2 px-3 text-gray-900 appearance-none cursor-pointer bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' fill=\'none\' viewBox=\'0 0 20 20\'%3E%3Cpath stroke=\'%236B7280\' stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'1.5\' d=\'m6 8 4 4 4-4\'/%3E%3C/svg%3E')] bg-[length:1.25em_1.25em] bg-[right_0.5rem_center] bg-no-repeat pr-10"
                                placeholder="סוג זמן"
                              >
                  ...
                  {time.timeType === 'fixed' ? (
                            <div>
                              <input
                                ...
                                onChange={(e) => handleUpdatePrayerTime(time.id, { fixedTime: e.target.value })}
                                className="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 py-2 px-3"
                              />
                            </div>
                  ) : (
                    <>
                            <div>
                              <select
                                ...
                                onChange={(e) => handleUpdatePrayerTime(time.id, { 
                                  relativeTime: { 
                                    ...time.relativeTime,
                                    relation: e.target.value as TimeRelation 
                                  }
                                })}
                                className="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white py-2 px-3 text-gray-900 appearance-none cursor-pointer bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg xmlns=\'http://www.w3.org/2000/svg\' fill=\'none\' viewBox=\'0 0 20 20\'%3E%3Cpath stroke=\'%236B7280\' stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'1.5\' d=\'m6 8 4 4 4-4\'/%3E%3C/svg%3E')] bg-[length:1.25em_1.25em] bg-[right_0.5rem_center] bg-no-repeat pr-10"
                                placeholder="יחס לזמן"
                              >
                                <option value="לפני השקיעה">לפני השקיעה</option>
                                <option value="אחרי השקיעה">אחרי השקיעה</option>
                                <option value="לפני הזריחה">לפני הזריחה</option>
                                <option value="אחרי הזריחה">אחרי הזריחה</option>
                              </select>
                            </div>
                            <div>
                              <input
